...
export async function saveCheckinDraft(
  ...
  if (error) {
    console.error('Failed to save check-in draft:', error.message);
  }
  return !error;
}
...
export async function loadCheckinDraft(
  ...
  if (error) {
    console.error('Failed to load check-in draft:', error.message);
  }
  if (!data) return null;
...
export async function clearCheckinDraft(
  ...
  try {
    ...
    // cleanup failure is not worth interrupting her — but it is worth a trace
