-- Daily bleeding/flow intensity for the pulse check-in.
-- NULL = not answered; 'none' = explicitly reported no bleeding.
ALTER TABLE public.symptom_checkins
  ADD COLUMN IF NOT EXISTS bleeding_flow TEXT DEFAULT NULL;

ALTER TABLE public.symptom_checkins
  DROP CONSTRAINT IF EXISTS bleeding_flow_values;

ALTER TABLE public.symptom_checkins
  ADD CONSTRAINT bleeding_flow_values
  CHECK (bleeding_flow IS NULL OR bleeding_flow IN ('none', 'spotting', 'light', 'moderate', 'heavy'));
